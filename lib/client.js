		...
		function buildCss(settings) {
			...
			const emit = (selector, style) => {
				...
				if (style.size > 0) {
					...
					rules.push(`${each(selector, " svg")} { width:${Math.round(style.size * .8)}px; height:${Math.round(style.size * .8)}px !important }`);
				}
				if (style.opacity !== void 0 && style.opacity !== 1) rules.push(`${each(selector, " *")} { opacity:${style.opacity} !important }`);
			};
					...
					const openPanel = () => {
						...
						modalSave = (s) => {
							try {
								...
								scope.set("thinking", s.thinking);
								scope.set("tool", s.tool);
							} catch (e) {
								console.warn("[dsh-skinskin] 保存失败", e);
							}
						};
